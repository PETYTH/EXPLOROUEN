...
export class ActivitiesService {
    static async getAllActivities(filters: ActivityFilters = {}, userId?: string) {
        ...
        let whereClause: any = {
            isActive: true
            // startDate: { gte: new Date() } // seulement futures si besoin
        };
        ...
        const activitiesWithDistance = (await Promise.all(activities.map(async activity => {
            let distance: number | null = null; // <- typé
            ...
            const userRegistration = userId
                ? registrations.find(reg => reg.userId === userId)
                : null;
            ...
            if (typeof filters.radius === 'number' && activity.distance !== null) {
    ...
    static async getActivityById(id: string, userId?: string) {
        ...
        const registrations = await prisma.registration.findMany({
            where: { itemId: id, type: 'ACTIVITY', status: 'ACCEPTED' },
    ...
    static async createActivity(data: CreateActivityData) {
        ...
        await prisma.discussion.create({
            data: { activityId: activity.id, title: `Discussion - ${activity.title}` }
        });
        ...
        if (data.places && data.places.length > 0) {
            ...
            await prisma.activityPlace.createMany({ data: activityPlaces });
        }
    ...
    static async registerToActivity(userId: string, activityId: string) {
        const activity = await prisma.activity.findUnique({
            where: { id: activityId },
            select: { id: true, startDate: true, maxParticipants: true }
        });
        if (!activity) throw new Error('Activité non trouvée');

        const registrationCount = await prisma.registration.count({
            where: { itemId: activityId, type: 'ACTIVITY', status: 'ACCEPTED' }
        });
        ...
        const existingRegistration = await prisma.registration.findUnique({
            where: { userId_itemId_type: { userId, itemId: activityId, type: 'ACTIVITY' } }
        });
        if (existingRegistration) throw new Error('Déjà inscrit à cette activité');

        const registration = await prisma.registration.create({
            data: { userId, itemId: activityId, type: 'ACTIVITY', status: 'ACCEPTED' }
        });

        let discussion = await prisma.discussion.findUnique({ where: { activityId } });
        if (!discussion) {
            discussion = await prisma.discussion.create({
                data: { activityId, title: `Chat - Activité ${activityId}` }
            });
        }
    ...
    static async unregisterFromActivity(userId: string, activityId: string) {
        ...
        if (!registration) throw new Error('Non inscrit à cette activité');

        const discussion = await prisma.discussion.findUnique({ where: { activityId } });
        if (discussion) {
        ...
        if (remainingRegistrations === 0 && discussion) {
            await prisma.discussionMessage.deleteMany({ where: { discussionId: discussion.id } });
    ...
    static async deleteActivity(activityId: string, _userId: string) {
        ...
        const registrations = await prisma.registration.findMany({
            where: { itemId: activityId, type: 'ACTIVITY' }
        });

        const participants = registrations.map((reg: any) => reg.userId);

        await prisma.discussion.deleteMany({ where: { activityId } });

        await prisma.discussionMessage.deleteMany({
            where: { discussion: { activityId } }
        });

        await prisma.registration.deleteMany({ where: { itemId: activityId, type: 'ACTIVITY' } });
    ...
    static async updateActivity(activityId: string, data: UpdateActivityData, userId: string) {
        ...
        if (!existingActivity) throw new Error('Activité non trouvée');
        ...
        if (!canModify) throw new Error('Vous n\'avez pas les droits pour modifier cette activité');
