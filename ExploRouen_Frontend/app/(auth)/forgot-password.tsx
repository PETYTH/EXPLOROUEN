...
export default function ForgotPasswordScreen() {
  ...
  const onRequestReset = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        'Si vous avez un compte, un email vous a été envoyé. Vérifiez aussi vos spams.',
        [{ text: 'Compris', onPress: () => codeRefs.current[0]?.focus() }]
      );
    } catch (err: any) {
      // Vérifier si c'est trop de tentatives
      if (err.errors?.length) {
        const ec = err.errors[0].code;
        if (ec === 'too_many_requests') {
          Alert.alert('Erreur', 'Trop de tentatives. Réessayez plus tard.');
          return;
        }
      }
      // Pour toutes les autres erreurs (y compris compte non trouvé), passer quand même à l'étape 2 pour la sécurité
      setCurrentStep(2);
      Alert.alert(
        'Vérifiez votre email', 
        'Si vous avez un compte, un email vous a été envoyé. Vérifiez aussi vos spams.',
  ...
  const handleCodeChange = (text: string, index: number) => {
    ...
    if (numeric.length <= 1) {
      ...
      // Si on ajoute un chiffre, passer au suivant
      if (numeric && index < 5) {
  ...
  // Handle backspace for cascade deletion
  const handleKeyPress = (key: string, index: number) => {
    if (key === 'Backspace') {
      const codeArray = code.split('');
      
      // Si l'input actuel a du contenu, le vider
      if (codeArray[index] && codeArray[index] !== '') {
        codeArray[index] = '';
        setCode(codeArray.join(''));
      } else {
        // Si l'input actuel est vide, aller au précédent et le vider
        if (index > 0) {
          codeArray[index - 1] = '';
          setCode(codeArray.join(''));
          codeRefs.current[index - 1]?.focus();
  ...
  const onResendCode = async () => {
    ...
    try {
      ...
      Alert.alert('Code renvoyé', 'Si vous avez un compte, un nouveau code vous a été envoyé.');
    } catch (err: any) {
      // Vérifier si c'est trop de tentatives
      if (err.errors?.length) {
        const ec = err.errors[0].code;
      ...
      // Pour toutes les autres erreurs (y compris compte non trouvé), afficher le même message pour la sécurité
      Alert.alert('Code renvoyé', 'Si vous avez un compte, un nouveau code vous a été envoyé.');
    }
  };
            ...
            <Animated.View style={[styles.contentContainer, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
              ...
              <View style={styles.formContainer}>
                  ...
                  <View style={styles.stepContainer}>
                    ...
                    <View style={styles.form}>
                      ...
                      <TouchableOpacity style={[styles.primaryButton, loading && styles.primaryButtonDisabled]} onPress={onRequestReset} disabled={loading}>
                        <View style={[styles.primaryButtonGradient, { backgroundColor: '#1E40AF' }]}>
                          {loading ? (
                ...
                {currentStep === 2 && (
                  <View style={styles.stepContainer}>
                    <View style={styles.iconContainer}>
                      <Ionicons name="shield-checkmark-outline" size={48} color="#1E40AF" />
                    </View>
                ...
                {currentStep === 3 && (
                  <View style={styles.stepContainer}>
                    <View style={styles.iconContainer}>
                      <Ionicons name="lock-closed-outline" size={48} color="#1E40AF" />
                    </View>
                    ...
                    <View style={styles.form}>
                      <View style={styles.passwordInputContainer}>
                        ...
                        <TouchableOpacity onPress={() => setShowPassword(!showPassword)} style={styles.eyeButton}>
                          <Ionicons name={showPassword ? 'eye-off-outline' : 'eye-outline'} size={20} color="#1E40AF" />
                        </TouchableOpacity>
                      </View>
                      ...
                      <View style={styles.passwordInputContainer}>
                        ...
                        <TouchableOpacity onPress={() => setShowConfirmPassword(!showConfirmPassword)} style={styles.eyeButton}>
                          <Ionicons name={showConfirmPassword ? 'eye-off-outline' : 'eye-outline'} size={20} color="#1E40AF" />
                        </TouchableOpacity>
                      </View>
...
const styles = StyleSheet.create({
  ...
  backButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#1E40AF', justifyContent: 'center', alignItems: 'center', marginBottom: 24 },
  ...
  modernProgressFill: { height: '100%', backgroundColor: '#1E40AF', borderRadius: 2 },
  ...
  progressStepActive: { backgroundColor: '#1E40AF', borderColor: '#1E40AF' },
  ...
  iconContainer: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(30,64,175,0.1)', justifyContent: 'center', alignItems: 'center', marginBottom: 24, borderWidth: 2, borderColor: 'rgba(30,64,175,0.3)' },
  ...
  modernCodeInputFilled: { borderColor: '#1E40AF', backgroundColor: '#FFFFFF' },
